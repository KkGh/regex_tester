import { Extension, RangeSetBuilder, StateEffect, StateEffectType, StateField, Transaction } from "@codemirror/state";
    ...
    const plugin = ViewPlugin.fromClass(
        class {
            ...
            update(update: ViewUpdate) {
                ...
                const isFieldChanged = update.startState.field(field) !== update.state.field(field);
            ...
            private createVisibleDeco(view: EditorView) {
                ...
                if (view.visibleRanges.length > 0) {
                    ...
                    const visibleDecos: { from: number, to: number, value: Decoration }[] = [];
                    decos.between(start, end, (from, to, value) => {
                        visibleDecos.push({ from, to, value });
                    });
                    visibleDecos.sort((a, b) => a.from - b.from)
                        .forEach(r => builder.add(r.from, r.to, r.value));

                    if (visibleDecos.length > 5000) {
                        console.warn(`too many decorations: ${visibleDecos.length} in ${decos.size}.`);
                    }
                }
