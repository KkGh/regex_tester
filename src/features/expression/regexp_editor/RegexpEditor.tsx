...
export const RegexpEditor = memo(
  ({ value, onChange, extensions, theme }: Props) => {
    const { pattern, flags } = value;

    const [errorMessage, setErrorMessage] = useState("");
    // console.log("RegexpEditor", pattern, flags);

    const handleRegexpChange = useCallback(
    ...
    return (
      <div>
        <InputGroup className="mb-2">
          ...
          <RegexpFlagMenu flags={flags} onChange={handleFlagsChange} />
        </InputGroup>
