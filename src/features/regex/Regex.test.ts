...
describe("regex matches() result", () => {
    ...
    it("contain Match", () => {
        const ms = Regex.matches(/bcd/gd, "abcdefg");
    ...
    it("contain Group array", () => {
        const ms = Regex.matches(/<(.+?) (.+?)>/gd, "<div class> <li style>");
    ...
    it("contain group with empty text when capturegroup didnt match", () => {
        const ms = Regex.matches(/(\d)?/gd, "apple");
...
describe("regex matches() by array", () => {
    it("contains groups", () => {
        const array = [..."<div class> <li style>".matchAll(/<(.+?) (.+?)>/gd)];
...
describe("regex pattern", () => {
    it("can contain special classes", () => {
        expect(Regex.matches(/\b/gd, "Hello World").length).toBe(4);
        expect(Regex.matches(/\n/gd, "Hello\nWorld").length).toBe(1);
        expect(Regex.matches(/\cI/gd, "Hello\tWorld").length).toBe(1);
        expect(Regex.matches(/\x09/gd, "Hello\tWorld").length).toBe(1);
        expect(Regex.matches(/\u3042/gd, "HelloあWorld").length).toBe(1);
        expect(Regex.matches(/\u{3042}/gdu, "HelloあWorld").length).toBe(1);
    });

    it("can contain assertions", () => {
        expect(Regex.matches(/Jack(?=Sprat)/gd, "JackBlack JackSprat").length).toBe(1);
        expect(Regex.matches(/\d+(?!\.)/gd, "123 45.67 .89")?.[0].text).toBe("123");
    });

    it("can contain (named) capture groups", () => {
        expect(Regex.matches(/(.+) (.+)/gd, "Hello World")?.[0].groups.length).toBe(2);
        expect(Regex.matches(/(?<first>.+) (?<second>.+)/gd, "Hello World")?.[0].groups.length).toBe(2);
    });
    ...
    it("can contain unicode property", () => {
        ...
        expect(Regex.matches(/\p{General_Category=Punctuation}+/gu, "Hello,World")?.[0].text).toBe(",");
        expect(Regex.matches(/\p{Script=Hiragana}+/gu, "Hello せかい")?.[0].text).toBe("せかい");
        expect(Regex.matches(/\p{Script_Extensions=Hiragana}+/gu, "Hello ワールド")?.[0].text).toBe("ー");

        // エイリアス
        expect(Regex.matches(/\p{gc=punct}+/gu, "Hello,World")?.[0].text).toBe(",");
        expect(Regex.matches(/\p{gc=P}+/gu, "Hello,World")?.[0].text).toBe(",");
        expect(Regex.matches(/\p{sc=Hira}+/gu, "Hello せかい")?.[0].text).toBe("せかい");
        ...
        // プロパティ名のみ、値はない
        expect(Regex.matches(/\p{Alphabetic}+/gu, "Hello,せかい")?.[0].text).toBe("Hello");
...
describe("replaceAllWithIndex", () => {
    it("returns indices", () => {
        const result = Regex.replaceAllWithIndex(/(\w+)/g, 'apple,banana,orange', '[$1]');
        ...
        expect(result).toEqual({
            ...
            replacements: [
                ...
                { oldValue: "orange", oldIndex: 13, newValue: "[orange]", newIndex: 17 },
            ]
        });
    });

    it("returns indices with lookaround", () => {
        const result = Regex.replaceAllWithIndex(/(?<=,)\w/g, 'ap#le,ban#na,ora#ge', '#');
...
describe("getPatternErrorReason", () => {
    it("returns reason", () => {
        ...
        expect(Regex.getRegExpSyntaxErrorReason(message)).toBe("Unterminated character class");
    });
});
