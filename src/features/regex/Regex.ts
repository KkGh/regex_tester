...
export class Regex {
    ...
    static extract(arg1: RegExpMatchArray[] | RegExp, arg2: string, arg3?: string): string {
        if (arg1 instanceof RegExp && typeof arg2 === "string" && typeof arg3 === "string") {
            const matches = Regex.doMatch(arg2, arg1);
    ...
    private static extractFromMatchResult(matches: RegExpMatchArray[], outputPattern: string) {
        let result = "";
    ...
    static replaceAllWithIndex(re: RegExp, str: string, replaceValue: string): ReplacementResult;
    static replaceAllWithIndex(matches: RegExpMatchArray[], str: string, replaceValue: string): ReplacementResult;

    /*
     * 正規表現に一致したすべての文字列を replaceValue で置き換えた新しい文字列と、
     * それぞれの replaceValue の文字列及び開始位置を返します。
     */
    static replaceAllWithIndex(arg1: RegExp | RegExpMatchArray[], str: string, replaceValue: string) {
        if (arg1 instanceof RegExp) {
            if (!arg1.global) throw new TypeError(`${Regex.replaceAllWithIndex.name} called with a non-global RegExp argument`);
            const matches = Regex.doMatch(str, arg1);
            return Regex.replaceAllWithIndexFromMatchResult(matches, str, replaceValue);
        }
        else {
    ...
    private static replaceAllWithIndexFromMatchResult(matches: RegExpMatchArray[], str: string, replaceValue: string) {
        let result = "";
        const replacements = [];
        let lastMatchEnd = 0;
        for (const m of matches) {
            ...
            const replacement = GetSubstitution(matched, str, position, captures, m.groups, replaceValue);
    ...
    static getRegExpSyntaxErrorReason(errorOrMessage: string | SyntaxError): string {
        const message = (errorOrMessage instanceof SyntaxError) ?
